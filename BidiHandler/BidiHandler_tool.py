...
body = open(input_file_path, 'r', encoding = 'utf-8').read()
handler = Bidi(body)
new_contnet = handler.RightToLeftEmbedding()
output_file_path = str(input_file_path).replace('.txt', '_handled.txt')
open(output_file_path, 'w', encoding = 'utf-8').write(new_contnet)